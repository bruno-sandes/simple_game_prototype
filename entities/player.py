"""entities/player.py — XP progressivo e upgrade_count."""
import math, pygame
from config import (PLAYER_SPEED, PLAYER_MAX_HP, PLAYER_SKILL_CD, PLAYER_INVINCIBLE,
                    ...
                    LANTERN_DEFAULT_RADIUS, XP_NEXT_BASE, XP_NEXT_MULT, WHITE, RED)
...
class Player(AnimatedSprite):
    def __init__(self, x, y, name="Heroi", color=(70,130,210)):
        ...
        self.xp             = 0
        self.level          = 1
        self.xp_next        = XP_NEXT_BASE   # 150 XP para nível 2
        self.upgrade_count  = 0              # compras feitas no mercador
        self._world         = None
    ...
    def use_skill(self, wx, wy):
        ...
        self.skills.append(Skill(self.x,self.y,dx/dist,dy/dist,
                                 damage=self.skill_damage,world=self._world,radius=self.skill_radius))
        self.skill_cd=self.skill_max_cd; return True

    def take_damage(self, amount):
        if self.invincible>0: return
        self.hp=max(0,self.hp-amount); self.invincible=PLAYER_INVINCIBLE; self.damage_flash=12

    def use_potion(self):
        if self.inventory.use_potion(): self.hp=min(self.max_hp,self.hp+40); return True
        return False
    ...
    def gain_xp(self, amount):
        ...
        while self.xp>=self.xp_next:
            ...
            self.xp_next=int(self.xp_next*XP_NEXT_MULT)   # x1.6 por nível
            self.inventory.add("coin",1); leveled=True
        return leveled
