"""ui/hud.py — HUD completo com floor, moedas, indicador de chave."""
import pygame
...
def draw_hud(surface, player, messages, floor=1):
    ...
    ready=player.skill_cd<=0; ratio=1.0-player.skill_cd/player.skill_max_cd
    pygame.draw.rect(surface,DARK_GRAY,(bx,by,bw,bh),border_radius=4)
    pygame.draw.rect(surface,LIGHT_BLUE if not ready else (100,255,180),(bx,by,int(bw*ratio),bh),border_radius=4)
    ...
    surface.blit(fonts.xs.render(f"Moedas:{coins}  Itens:{player.inventory.total}  [I]",True,LIGHT_GRAY),(10,72))
    # Andar
